...
export class Home extends Component {
    ...
    mounted(){
        articleService
            ...
            .then(articles => {
                ...
                this.articles.sort((a,b) => (a.creation_date < b.creation_date) ? 1 : ((b.creation_date < a.creation_date) ? -1 : 0));
                this.articlesSliced = this.articles.slice(1, 5);
            })
            ...
            <div>
                ...
                <div id="card-column-wrapper">
                    <div className="card-columns">
                        {this.articlesSliced.map(s => <Card key={s.id} title={s.title} image={s.image} id={s.id} alt={s.alt}/>)
                        }
                    </div>
                </div>
                <div className="textAlignCenter">
                   <Button.Round onClick={this.moreArticles}>Load more articles</Button.Round>
                </div>
            </div>
        )
    }
    ...
    moreArticles(){
        this.articlesSliced = this.articles.slice(1, 5 + this.sliceFactor);
        this.sliceFactor += 5;
    }
}
